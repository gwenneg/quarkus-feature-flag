 ...
 * </pre>
 */
public class FeatureFlagsLoggerCreator implements Consumer<MethodCreator> {

    private final String className;
    private final List<FieldInfo> flags;

    public FeatureFlagsLoggerCreator(String className, List<FieldInfo> flags) {
    ...
    public void accept(MethodCreator mc) {
        ...
        AssignableResultHandle loggerHandle = mc.createVariable(Logger.class);
        ...
        for (FieldInfo flag : flags) {
            ResultHandle formatHandle = mc.load(getFlagMessageFormat(flag));
            ResultHandle fieldHandle = mc.readInstanceField(FieldDescriptor.of(flag), featureFlagsClassHandle);
